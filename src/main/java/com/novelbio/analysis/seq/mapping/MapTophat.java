 ...
 * 还没返回结果的bam文件
 * tophat的mapping
 * 	 * tophat -r 120 -a 10 -m 1 -i 20 -I 6000 --solexa1.3-quals -F 0.15 -p 4 --coverage-search 
	 * --min-coverage-intron 20 --max-coverage-intron 	6000 --min-segment-intron 20 
	 * --max-segment-intron 6000 -G /media/winE/Bioinformatics/GenomeData/Arabidopsis\ TAIR9/TAIR10GFF/TAIR10_GTF3_genes.gtf  
	 * -o /media/winE/NBC/Project/RNASeq_GF110614/rawdata/tophatResult/ /media/winE/Bioinformatics/GenomeData/Arabidopsis\ TAIR9/ChromFa/TAIR10_ath
	 *  /media/winE/NBC/Project/RNASeq_GF110614/rawdata/data/Col_L1_1.fq,/media/winE/NBC/Project/RNASeq_GF110614/rawdata/data/Col_L2_1.fq /media/winE/NBC/Project/RNASeq_GF110614/rawdata/data/Col_L1_2.fq,/media/winE/NBC/Project/RNASeq_GF110614/rawdata/data/Col_L2_2.fq
 * @author zong0jie
 *
 */
public class MapTophat {
	 ...
	 * @param bowtie2
	 */
	public void setBowtie2(boolean bowtie2) {
		bowtie2Mapping = bowtie2;
	}
	/** 是否使用bowtie2进行分析 */
	private String getBowtie1() {
		if (bowtie2Mapping) {
			return "";
		}
		return " --bowtie1 ";
	...
	private String getMinCoverageIntron() {
		if (intronLenMin < 50) {
			return "--min-coverage-intron " + intronLenMin + " --min-segment-intron " + intronLenMin + " ";
		}
		return "";
	}
	 ...
	 * @param strandSpecifictype<br>
	 * <b>fr-unstranded</b>	Standard Illumina	Reads from the left-most end of the fragment (in transcript coordinates) map to the transcript strand, 
	 * and the right-most end maps to the opposite strand.<br>
       <b>fr-firststrand</b>	dUTP, NSR, NNSR	Same as above except we enforce the rule that the right-most end of the fragment
        (in transcript coordinates) is the first sequenced (or only sequenced for single-end reads). 
        Equivalently, it is assumed that only the strand generated during first strand synthesis is sequenced.<br>
<b>fr-secondstrand</b>	Ligation, Standard SOLiD	Same as above except we enforce the rule that the left-most end of the fragment (in transcript coordinates) is the first sequenced (or only sequenced for single-end reads). Equivalently, it is assumed that only the strand generated during second strand synthesis is sequenced.
	 */
	...
	/**
	 * 参数设定不能用于solid
	 * 还没加入gtf的选项，也就是默认没有gtf
	 */
	public SamFile mapReads() {
		IndexMakeBowtie(bowtie2Mapping);
...
//		linux命令如下
		...
		* tophat -r 120 -a 10 -m 1 -i 20 -I 6000 --solexa1.3-quals -F 0.15 -p 4 --coverage-search 
		...
		String cmd = ""; cmd = ExePathBowtie + "tophat " + getBowtie1();
		if (pairend) {
			cmd = cmd + getInsert(); //插入长度
		}
		cmd = cmd + getAnchoLen() + getAnchorMismatch() + getIntronLenMin() + getIntronLenMax() + getIndelLen();
		if (bowtie2Mapping) {
			cmd = cmd + getIndelLen();
		}
	 ...
	 * @param bowtie2
	 */
	private void IndexMakeBowtie(boolean bowtie2) {
//		linux命令如下 
...
//		-c 是solid用
		...
		String cmd = "";
		if (bowtie2) {
			cmd = this.ExePathBowtie + "bowtie2-build ";
		}
		else {
			cmd = this.ExePathBowtie + "bowtie-build ";
		}
		//TODO :考虑是否自动判断为solid
