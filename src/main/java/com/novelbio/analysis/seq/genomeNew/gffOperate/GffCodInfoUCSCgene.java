...
public class GffCodInfoUCSCgene extends GffCodAbs
{
	 ...
	 */
	@Override
	protected void SearchLOCinside(ArrayList<GffDetail> loclist, int beginnum, int endnum)
	{
		GffDetailUCSCgene LOCdetial=(GffDetailUCSCgene) loclist.get(beginnum);
		result=true;//找到位置
		begincis5to3=LOCdetial.cis5to3;//本基因方向
		insideLOC=true;//在基因内
		...
		if(LOCdetial.cis5to3)
		{
			...
			distancetoLOCEnd[0]=Math.abs(Coordinate-LOCdetial.numberend);//到本基因终点的位置
		}	
		else {
			distancetoLOCStart[0]=Math.abs(Coordinate-LOCdetial.numberend);//到本基因起点的位置
			distancetoLOCEnd[0]=Math.abs(Coordinate-LOCdetial.numberstart);//到本基因终点的位置
		}
		...
		codToATG[0]=1000000000;
		codToATG[1]=1000000000;
		
		
		
		flag=false;
		if (begincis5to3 ) 
			codToATG[0]=Coordinate-longSplitExon.get(0);
		else if (!begincis5to3)
			codToATG[0]=longSplitExon.get(1)-Coordinate;
			...
			for(int j=2; j<exonNum; j++)  //一个一个Exon的检查
			{
				if(Coordinate<longSplitExon.get(j) && j%2==0)//在外显子之前（内含子中），外显子为： 2,3  4,5  6,7  8,9   0该转录本的转录起点，1该转录本的转录终点
				{
				   ...
				   if(j==2)//在5‘UTR中,也算做在外显子中   tss cod ，2 3 ， 4 5 ， 6 7 ， 8 9 
				   {
					   ...
					   addingeneinfo(splitID, position, ExIntronnum, start, end,UTR,UTRstart,UTRend);//装入list
					   break;//跳出	   
				   }
				   else //在内含子中
				   {
	 ...
	 */
	@Override
	protected void SearchLOCoutside(ArrayList<GffDetail> Genlist, int beginnum,
			int endnum) {
		GffDetailUCSCgene endnumlist = null;
		GffDetailUCSCgene beginnumlist = null;

		result = true;// 找到位置
		insideLOC = false;// 在基因间
		...
		if (beginnum != -1) {
			...
			begincis5to3 = beginnumlist.cis5to3;// 上个基因方向
			LOCID[1] = beginnumlist.locString;// 上个基因的ID
			// 与前一个基因转录起点和终点的距离
			if (begincis5to3) {// 当基因正向时，与TSS距离为正数，与End为负数 |>----->------*
				distancetoLOCStart[0] = Math.abs(Coordinate
						- beginnumlist.numberstart);
				distancetoLOCEnd[0] = -Math.abs(Coordinate
						- beginnumlist.numberend);
			} else {// 当基因反向时，与TSS距离为负数，与End为正数 <-------<|----*
				distancetoLOCStart[0] = -Math.abs(beginnumlist.numberend
						- Coordinate);
				distancetoLOCEnd[0] = Math.abs(Coordinate
						- beginnumlist.numberstart);
			}
		...
		if (endnum != -1) {
			...
			endcis5to3 = endnumlist.cis5to3;// 下个基因方向
			LOCID[2] = endnumlist.locString;// 下个基因的ID
			// 与后一个基因转录起点和终点的距离
			// 与后一个基因转录起点和终点的距离
			if (endcis5to3) {// 当基因正向时，与TSS距离为负数，与End为正数 *---|>----->----
				distancetoLOCStart[1] = -Math.abs(Coordinate
						- endnumlist.numberstart);
				distancetoLOCEnd[1] = Math.abs(Coordinate
						- endnumlist.numberend);
			} else {// 当基因反向时，与TSS距离为正数，与End为负数 *----<-------<|
				distancetoLOCStart[1] = Math.abs(endnumlist.numberend
						- Coordinate);
				distancetoLOCEnd[1] = -Math.abs(Coordinate
						- endnumlist.numberstart);
			}
			...
			if (begincis5to3)
				codToATG[1] = Coordinate - longSplitExon.get(0);
			else if (!begincis5to3)
				codToATG[1] = longSplitExon.get(1) - Coordinate;
		}
	}
