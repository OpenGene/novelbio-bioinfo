...
/**
 * 本类用来将染色体的名字，序列装入染色体类，并且是以Hash表形式返回
 * 目前本类中仅仅含有静态方法
 * 同时用来提取某段位置的序列
 * 和提取反向重复序列
 * 作者：宗杰 20090617
...
public class ChrStringHash {
	...
	private static ChrStringHash chrStringHash = null;
	
	
	
	
	/**
	 * 将染色体信息读入哈希表,按照RandomAccessFile保存，并返回
	 ...
	 * @throws FileNotFoundException
	  */
	 public void setChrFilePath(String chrFilePath) throws Exception
	 {
		 if (compMap==null) {
			 compmapFill();
		}
	 ...
	 /**
	  * 当设定Chr文件后，可以将序列长度输出到文件
	  * 输出文件为  chrID(小写)+“\t”+chrLength+换行
	  * 不是顺序输出
	 ...
	 * @throws IOException 
	 ...
	 public void saveChrLengthToFile(String outFile)  
	 {
		 ...
		 * startNum=204;从第几个碱基开始读取，从1开始记数，注意234的话，实际为从234开始读取，类似substring方法
		 * long endNum=254;//读到第几个碱基，从1开始记数，实际读到第endNum个碱基。
		 * 快速提取序列
		 * @throws IOException
		 */
		private String getSeq(String chrID,long startlocation,long endlocation) throws IOException 
		{
			startlocation--;
	...
	/**
	 * 	 * 输入染色体list信息
	 * 输入序列坐标以及是否为反向互补,其中ChrID为 chr1，chr2，chr10类型
	 * 返回序列
	 * @param cisseq 正反向
	 ...
	 * 获得反向互补序列
	 */
	public String resCompSeq(String sequence,HashMap<Character, Character> complementmap)
	{
