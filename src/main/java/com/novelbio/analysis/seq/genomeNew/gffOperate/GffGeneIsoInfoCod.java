...
public abstract class GffGeneIsoInfoCod<T extends GffGeneIsoInfo> {
	T gffGeneIso = null;
	...
	{
		if (codLoc == COD_LOC_OUT && getCod2Tss() < 0 && Math.abs(getCod2Tss()) <= T.UpStreamTSSbp ) {
			return true;
		}
		else if ( codLoc != COD_LOC_OUT && getCod2Tss() > 0 && Math.abs(getCod2Tss()) <= T.DownStreamTssbp ) {
			return true;
		}
		return false;
	}
	...
	{
		if (codLoc == COD_LOC_OUT && getCod2Tes() > 0 && Math.abs(getCod2Tes()) <= T.GeneEnd3UTR ) {
			return true;
		}
		return false;
	}
...
//	HashMap<Integer, Integer> hashLocExInStart;
	...
	public String getCodLocStr() {
		...
		if (isCodInIsoTss() && getCodLoc() == COD_LOC_OUT) {
			if (cod2TSS > T.PROMOTER_INTERGENIC_MAMMUM) {
				result = T.PROMOTER_INTERGENIC_STR;
			}
			else if (cod2TSS > T.PROMOTER_DISTAL_MAMMUM) {
				result = T.PROMOTER_DISTAL_STR;
			}
			else {
				result = T.PROMOTER_PROXIMAL_STR;;
			}
		}
		else if (isCodInIsoTss() && getCodLoc() != COD_LOC_OUT) {
			result = T.PROMOTER_DOWNSTREAMTSS_STR;
		}
