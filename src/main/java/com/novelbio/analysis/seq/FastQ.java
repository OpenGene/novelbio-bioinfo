...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	private void setHashFastQFilter(int QUALITY) {
		...
		else if (QUALITY == QUALITY_MIDIAN || QUALITY == QUALITY_MIDIAN_PAIREND) {
			quality = QUALITY;
			hashFastQFilter.put(2, 1);
	...
	public int guessFastOFormat(List<String> lsFastQ) {
		...
		logger.error(seqFile + " has a problem, FastQ can not gess the fastQ format, set the format as FASTQ_ILLUMINA_OFFSET");
	 ...
	 * @param outFilePrix
	 * @param barcodeAndName 一个barcode序列--自动转换为大写，一个barcode名字
	 * 所以barcodeAndName必须是一个偶数长度的数组
	 */
	...
	/**
	 * 指定输出的文件全名，自动将路径最后的文件名设置为前缀
	 * @param OutPrix 输出文件全名
	 * @param maxmismatch barcode 最大错配
	 * @param barcodeAndName 一个barcode序列--自动转换为大写，一个barcode名字
	 * @throws Exception 
	 */
	public void filterBarcode(String OutFilePathPrix,int maxmismatch,String...barcodeAndName) throws Exception 
	{
		...
		{
			String barcodeseq = entry.getKey();	String barcodename = entry.getValue();
			TxtReadandWrite txtBarcod = new TxtReadandWrite();
			txtBarcod.setParameter(filePath+fileName[0]+"_"+barcodename+"."+fileName[1], true, false);
			hashBarcodeTxt.put(barcodename, txtBarcod);
			if (booPairEnd) {
				TxtReadandWrite txtBarcod2 = new TxtReadandWrite();
				txtBarcod2.setParameter(filePath+fileName[0]+"_"+barcodename+"1."+fileName[1], true, false);
	...
	/**
	 * 输入一个一维数组，将其装入HashBarCode表
	 * 数组格式为 0：barcode 序列 1：barcode对应的名称
	 * 2：barcode 序列 3：barcode对应的名称
	 */
	private void setHashBarCode(String[] barcodeAndName) {
		...
		for (int i = 0; i < barcodeAndName.length - 1; i = i+2) {
			hashBarcodeName.put(barcodeAndName[i].trim().toUpperCase(), barcodeAndName[i+1].trim());
	 ...
	 * @param barcodeSeq
	 * @param maxmismatch 允许barcode最大错配数
	...
	public void WZFsepFastQ(String outFile) throws Exception {
		...
		String content = "";
		String out1 = ""; String out2 = ""; int flag = 1;//标记是1端还是2端
		...
		while ((content = reader.readLine()) != null) {
			if (count == 0)
			{
				...
				if (content.endsWith("/2")) 
				{
					...
					if (!con1.substring(0, con1.length()-1).equals(content.substring(0,content.length()-1))) {
						logger.error("/1 and /2 are not equal"+out1.split("\n")[0]+content );
						out1 = "";
						out2 = "";
						continue;
					}
