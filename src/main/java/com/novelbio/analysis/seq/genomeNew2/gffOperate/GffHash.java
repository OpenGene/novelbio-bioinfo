...
GffHash <T extends GffDetailAbs, K extends GffCodAbs<N, T>, M extends GffCodAbsDu<T,K, N>, N extends GffDetailAbsCod<T>>{
	/**
	 * 起点默认为开区间
	 */
	int startRegion = 1;
	...
	public K searchLocation(String chrID, int Coordinate) {
		...
		{
			gffCod.gffDetailThis = (N) Loclist.get(locInfo[1]).setCood(Coordinate);
			gffCod.booFindCod = true;
			gffCod.ChrHashListNumThis = locInfo[1];
			gffCod.insideLOC = true;
			if (locInfo[1] - 1 >= 0) {
				gffCod.gffDetailUp =  (N) Loclist.get(locInfo[1]-1).setCood(Coordinate);
				gffCod.ChrHashListNumUp = locInfo[1]-1;
			}
			if (locInfo[2] != -1) {
				gffCod.gffDetailDown = (N) Loclist.get(locInfo[2]).setCood(Coordinate);
				gffCod.ChrHashListNumDown = locInfo[2];
			}
		} else if (locInfo[0] == 2) {
			gffCod.insideLOC = false;
			if (locInfo[1] >= 0) {
				gffCod.gffDetailUp = (N) Loclist.get(locInfo[1]).setCood(Coordinate);
