...
public class CufflinksGTF {
	 ...
	 * @param fqFile
	 */
	public void setBam(String... samBamFile) {
		lsSamFiles.clear();
		for (String sambamFile : samBamFile) {
			SamFile samFile = new SamFile(sambamFile);
			lsSamFiles.add(samFile);
		}
	}

	/** 在junction 的一头搭上exon的最短比例
	 * 0-1之间，默认0.09
	 *  */
	...
	private void setGTFfile() {
		...
		if (gffChrAbs != null && gffChrAbs.getGffHashGene() != null) {
			String path = FileOperate.getParentPathName(lsSamFiles.get(0).getFileName());
	...
	public void runCufflinks() {
		...
		// linux命令如下
		/**
		 cufflinks -o /media/winE/NBC/Project/RNASeq_GF110614/resultTmp/cufflinks -p 4 
		 -G /media/winE/Bioinformatics/GenomeData/Arabidopsis\ TAIR9/TAIR10GFF/TAIR10_GTF3_genes.gtf 
		 -b /media/winE/Bioinformatics/GenomeData/Arabidopsis\ TAIR9/ChromFa/TAIR10_chr_All.fas -u 
		 -N --compatible-hits-norm -L COL -I 15000 --min-intron-length 20
		  /media/winE/NBC/Project/RNASeq_GF110614/resultTmp/tophatResult/accepted_hits.bam
		 */
		String cmd = "";
		cmd = ExePathCufflinks + "cufflinks ";
		cmd = cmd + getOutPathPrefix() + getAnchoProportion() + getIntronLenMin() + getIntronLenMax() + getGtfFile() + getStrandSpecifictype() + getThreadNum() + getCorrectChrFile();
