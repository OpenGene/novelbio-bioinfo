 ...
 */
 public class Vocalno {
	...
	public Vocalno(String excelName, String[] compare) {
		this.excelFileName = excelName;
		
		lslsInfo = ExcelTxtRead.readLsExcelTxt(excelName, 0);
		QUANUM = lslsInfo.size()/10;
		String[] titles = lslsInfo.get(0);
		fdrCol = findColNum(titles, TitleFormatNBC.FDR.toString());
		pvalueCol = findColNum(titles, TitleFormatNBC.Pvalue.toString());
		logfcCol = findColNum(titles, TitleFormatNBC.Log2FC.toString());
	 ...
	 * @return 返回从0开始的计数
	 */
	private int findColNum(String[] titles, String colName) {
		int colNum = -1;
		for (int i = 0; i < titles.length; i++) {
			if (titles[i].equalsIgnoreCase(colName)) {
				colNum = i;
			}
		}
		return colNum;
	...
	public static List<Double> readListListCol(List<String[]> lsls, int colNum, double inf, double naValue) {
		...
		for (String[] values : lsls) {
			String str = values[colNum];
			double value;
			//TODO 尚未考虑NA等情况
	 ...
	 * 1：具体的阈值
	 */
	public static String[] setThreshold(Collection<Vocalno> lslspValue, double upfc, double downfc) {
		int thresholdNum = (int)(lslspValue.size() * MIN_PROP_NUM + 1);
		/** 超过1000个基因的样本数量 */
		int pvalueUpThresh1 = 0, pvalueUpThresh2 = 0, fdrUpThresh1 = 0, fdrUpThresh2 = 0;
		for (Vocalno difResultInfo : lslspValue) {
