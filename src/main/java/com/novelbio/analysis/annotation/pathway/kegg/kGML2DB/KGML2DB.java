...
public class KGML2DB {
	private static final Logger logger = Logger.getLogger(KGML2DB.class);
	/**
	 * 将KGML导入数据库
	 * @param filePath
	 * @throws Exception
	...
	public static void readKGML(String filePath) {
		ArrayList<String> lsKGML=FileOperate.getFoldFileNameLs(filePath, "*", "xml");
		Serializer serializer = new Persister();
		for (String fileName : lsKGML) {
			logger.error("readingFile:" + " " + fileName);
			KGML example = null;
			try {
				example = serializer.read(KGML.class,  new File(fileName));
			} catch (Exception e) {
				logger.error("文件出错："+ fileName);
				continue;
			}
			kgml2DB(example);
		}
	...
	private static void kgml2DB(KGML kgml) {
		...
		TaxInfo taxInfo = servTaxID.queryAbbr(taxAbbr);
		int taxID = 0;
		if (taxInfo != null) {
			taxID = taxInfo.getTaxID();
		}
		...
		for (Entry entry : lsEntry) {
			...
			kGentry.setID(entry.getID());
			kGentry.setType(entry.getType());
			kGentry.setLinkEntry(entry.getLinkEntry());
			if (kGentry.getType().equals("group")) {
				kGentry.setEntryName(entry.getEntryName());//仅仅是为了下面检查名字是不是"undefined"
				...
				if (!kGentry.getEntryName().equals("undefined")) {
					...
					int compNum= entry.getLsComponent().size();
					for (int j = 0; j < compNum; j++)
					{
						int entryID=entry.getLsComponent().get(j).getComID();//获得该component中一个entry的ID
						//获得那个子类的entryID
						...
						if (!entry.getReaction().equals("")) {
							ss2 = entry.getReaction().trim().split(" +"); 
						}
						//不知道会不会有这种情况，group中的reaction和其中的component的reaction不一致，这个就是来测试的，如果发现不一致了，那么就报错。
						if (!entryComp.getReaction().equals("")&&
								!entry.getReaction().equals("")
										&&!entry.getReaction().equals(    entryComp.getReaction()   	)
								)
						{
