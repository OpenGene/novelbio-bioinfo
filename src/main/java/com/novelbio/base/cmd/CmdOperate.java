...
//		ProgressData progressDataIn = new ProgressData();
	...
	public void doInBackground(String cmdFileName) {
		logger.info(cmd);
		String cmd1SH = NovelBioConst.PATH_POSITION_RELATE + cmdFileName
				+ DateTime.getDate() + ".sh";
	 ...
	 *            文件名，备份用的，不需要路径
	 */
	public void doInBackgroundR(String cmdFileName) {
		logger.info(cmd);
		String cmd1SH = NovelBioConst.PATH_POSITION_RELATE + cmdFileName
				+ DateTime.getDateMM() + ".R";
		TxtReadandWrite txtCmd1 = new TxtReadandWrite(cmd1SH, true);
		txtCmd1.writefile(cmd);
		txtCmd1.close();
		cmd = NovelBioConst.R_SCRIPT + cmd1SH;
		try {
			doInBackgroundB();
		} catch (Exception e) {
			logger.error("cmd cannot executed correctly: " + cmd);
