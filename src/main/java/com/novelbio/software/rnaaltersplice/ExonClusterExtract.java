...
public class ExonClusterExtract {
	 ...
	 */
	public ExonClusterExtract(GffGene gene) {
		this.gene = gene;
	}
	...
	public List<ExonClusterSite> getLsDifExonSite() {
		List<GffIso> lsSameGroupIso = getLsGffGeneIsoSameGroup();
		/**
	 ...
	 */
	private List<ExonClusterSite> addExonCluster(List<GffIso> lsSameGroupIso ) {
	...
	private void getLsExonInRegionRecur(ExonClusterSite exonClusterSite, Boolean cis5to3, 
			List<ExonCluster> lsExonClusterLong, List<GffIso> lsIso) {
		...
		for (ExonCluster exonCluster : lsExonClusterLong) {
			...
			List<GffIso> lsIsoRemoveLong = removeLongExon(isRetainIntron, exonCluster, lsIso);
	 ...
	 */
	private List<ExonCluster> getLsExonClusterInRegion(Alignment align, Boolean cis5to3, List<GffIso> lsSameGroupIso) {
	...
	private List<GffIso> removeLongExon(boolean isRetainIntron, ExonCluster exonCluster, List<GffIso> lsIsoRaw) {
		if (isRetainIntron) {
	 ...
	 * @return
	 */
	private ArrayList<GffIso> removeLongExonRetainIntron(ExonCluster exonCluster, List<GffIso> lsIsoRaw) {
		...
		HashSet<GffIso> setGeneIsoWithLongExon = new HashSet<GffIso>();
		...
		ArrayList<GffIso> lsSameGroupIsoNew = new ArrayList<GffIso>();
		for (GffIso gffGeneIsoInfo : lsIsoRaw) {
	 ...
	 * @return
	 */
	private ArrayList<GffIso> removeLongExonOther(ExonCluster exonCluster, List<GffIso> lsIsoRaw) {
		...
		//----------------------------------15==20-------30===40
		//这时候就把第二个转录本也删了
		Set<GffIso> setIsoNeedToRemove = new HashSet<>();
	 ...
	 *  */
	private List<GffIso> getLsGffGeneIsoSameGroup() {
		...
		if (gene.getLsCodSplit().size() <= 3) {
			ArrayList<GffIso> lsResult = new ArrayList<GffIso>();
		...
		List<GffIso> lsGeneIsoInfos = new ArrayList<>(gene.getLsCodSplit());
...
class IsoGroup {
	List<GffIso> lsGffGeneIsoInfos = new ArrayList<>();
	Set<Integer> setEdge = new HashSet<>();
	
	public int getSameEdge(GffIso gffGeneIsoInfo) {
		int i = 0;
	...
	public double getSameEdgeProp(GffIso gffGeneIsoInfo) {
