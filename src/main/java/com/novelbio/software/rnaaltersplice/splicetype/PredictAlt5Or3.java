...
public abstract class PredictAlt5Or3 extends SpliceTypePredict {
	Map<Integer, GffIso> mapEdge2Iso;
	...
	protected ArrayListMultimap<String, Double> getLsJuncCounts(String condition) {
		...
		//第一个剪接点
		List<JunctionUnit> lsJunctionStart = tophatJunction.getLsJunctionUnit(condition, exonCluster.isCis5to3(), exonCluster.getChrId(), align.getStartAbs());
		//第二个剪接点
		List<JunctionUnit> lsJunctionEnd = tophatJunction.getLsJunctionUnit(condition, exonCluster.isCis5to3(), exonCluster.getChrId(), align.getEndAbs());
		...
		for (JunctionUnit junctionUnit : lsJunctionStart) {
			...
			Map<String, Double> mapGroup2ValueTmp1 = tophatJunction.getJunctionSite(condition, exonCluster.isCis5to3(),
					exonCluster.getChrId(), junctionUnit.getStartAbs(), junctionUnit.getEndAbs());
...
//		Map<String, Double> mapGroup2Value2 = tophatJunction.getJunctionSite(condition, exonCluster.isCis5to3(), exonCluster.getRefID(), align.getEndAbs());
		if (mapGroup2ValueEndSite.size() == 0) {
			mapGroup2ValueEndSite = tophatJunction.getJunctionSite(condition, exonCluster.isCis5to3(), exonCluster.getChrId(), align.getEndAbs());
		}
	...
	public List<Align> getDifSite() {
		...
		for (Integer edge : mapEdge2Iso.keySet()) {
			double juncNum = tophatJunction.getJunctionSiteAll(exonCluster.isCis5to3(), exonCluster.getChrId(), edge);
		...
		Align align = new Align(exonCluster.getChrId(), MathComput.min(startEnd), MathComput.max(startEnd));
