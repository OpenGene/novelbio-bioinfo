...
public class GffCompareUCSCandNCBI {
	...
	private void compare(String outFile) {
		...
		for (GffGene gffDetailGeneUCSC : gffHashGeneUCSC.getGffDetailAll()) {
			GffIso gffGeneIsoInfoUCSC = gffDetailGeneUCSC.getLongestSplitMrna();
			GffIso gffGeneIsoInfoNCBI = gffHashGeneNCBI.searchISO(gffGeneIsoInfoUCSC.getName());
			...
			if (gffGeneIsoInfoUCSC.getName().startsWith("NR_") || gffGeneIsoInfoUCSC.getATGsite() == gffGeneIsoInfoNCBI.getATGsite() || gffGeneIsoInfoUCSC.getUAGsite() == gffGeneIsoInfoNCBI.getUAGsite()) {
				...
				txtOut.writefileln("出现不一致的Iso\t" +gffGeneIsoInfoNCBI.getParentGffDetailGene().getChrId() + "\t" + gffGeneIsoInfoNCBI.getName());
