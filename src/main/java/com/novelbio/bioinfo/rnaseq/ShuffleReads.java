...
public class ShuffleReads {
				    ...
				    private Align getChrIdRandom() {
				    	...
				    	do {
				        	...
				        	if (histBin.getCountNumber() < histBin.getLength()) {
				    			String chrId = histBin.getName();
				    			align = getAlign(chrId, (int) histBin.getCountNumber());
				    			
				    			histBin.addNumber();
								readNum[0]++;
				    		} else {
				    			allReadsNumTmp[0] = removeChrId(histBin.getName());
							}
						} while (align == null);
				    	return align;
				    }
	...
	private int removeChrId(String chrId) {
		...
		for (HistBin histBin : lsChrId2ReadsNum) {
			if (histBin.getName().equals(chrId)) {
		...
		for (int j = 0; j < lsChrId2ReadsNum.size(); j++) {
			...
			if (j == 0) {
				lsChrId2ReadsNumTmp.setStartBin(0, histBin.getName(), 0, allReads);
			} else {
				lsChrId2ReadsNumTmp.addHistBin(0, histBin.getName(), allReads);
			}
		}
