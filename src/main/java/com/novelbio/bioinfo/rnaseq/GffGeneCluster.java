...
public class GffGeneCluster {
	 ...
	 */
	ArrayList<ArrayList<GffGene>> lsGeneCluster = new ArrayList<ArrayList<GffGene>>();
	/**依次添加的GffHash的名字 */
	 ...
	 */
	public void addLsGffDetailGene(String gffFileName, ArrayList<GffGene> lsGffDetailGenes) {
		lsGeneCluster.add(lsGffDetailGenes);
		lsListGffName.add(gffFileName);
	}
	/** 当输入了Ref和This两个GffHash的时候才能用 */
	public ArrayList<GffGene> getThisGffGene() {
		setRefGffGene();
		if (isContainsRef) {
			if (lsGeneCluster.size() == 0) {
				return new ArrayList<GffGene>();
			} else {
	...
	public ArrayList<GffGene> getRefGffGene() {
		setRefGffGene();
		if (isContainsRef) {
			return lsGenesRef;
		}
	 ...
	 * @return
	 */
	public List<GffGene> getCombinedGffGene() {
	...
	private int getLongestGffGene_In_LsGeneCluster() {
		...
		for (int i = 0; i < lsGeneCluster.size(); i++) {
			for (GffGene gffDetailGene : lsGeneCluster.get(i)) {
...
//	}
	//TODO
	private List<GffGene> compareAndModify_GffGeneNew() {
		ListGff lsGffDetailGenes = new ListGff();
		GffGene gffDetailGeneRef = getGffDetailGene(lsGenesRef);
		GffGene gffDetailGeneResult = gffDetailGeneRef.clone();
		gffDetailGeneResult.clearIso();
		HashSet<String> setGffIsoRefSelectName = new HashSet<String>();//所有选中的Iso的名字，也就是与cufflink预测的转录本相似的转录本
		//这里的lsGeneCluster已经去除了refGff
		for (ArrayList<GffGene> lsgffArrayList : lsGeneCluster) {//获得GffCluster里面每个GffHash的list，这个一般只有一个GffHash
			GffGene gffDetailGeneCalculate = getGffDetailGene(lsgffArrayList);				
	...
	private GffGene getGffDetailGene(List<GffGene> lsGffDetailGenes) {
		GffGene gffDetailGene = lsGffDetailGenes.get(0).clone();
		for (int i = 1; i < lsGffDetailGenes.size(); i++) {
			GffGene gffDetailGene2 = lsGffDetailGenes.get(i);
	 ...
	 */
	private Set<String> modifyGffRef(GffGene gffDetailGeneResult, GffGene gffDetailGeneRef, GffGene gffDetailGeneCalculate) {
		Set<String> setGffIsoRefSelectName = new HashSet<>();
		for (GffIso gffIsoThis : gffDetailGeneCalculate.getLsCodSplit()) {//遍历该GffDetailGene的转录本，并挑选出最接近的进行比较	
			//选择了仅仅是起点和终点不同的exon，方便修正
			GffIso gffIsoRef = gffDetailGeneRef.getAlmostSameIso(gffIsoThis);
			//TODO 开始做修正工作
			if (gffIsoRef == null) {
				GffIso gffIsoSimilar = gffDetailGeneRef.getSimilarIso(gffIsoThis, 0.5);
			...
			GffIso gffIsoTmpResult = compareIso(gffIsoRef, gffIsoThis);
	 ...
	 */
	public GffIso compareIso(GffIso gffGeneIsoInfoRef, GffIso gffGeneIsoInfoThis) {		
		GffIso gffGeneIsoInfoResult = gffGeneIsoInfoRef.clone();
...
class ExonClusterBoundInfo {
	GffIso gffGeneIsoInfoRef;
	GffIso gffGeneIsoInfoThis;
	/** 上一个位点的状态信息 */
	...
	public ExonClusterBoundInfo(GffIso gffGeneIsoInfoRef, GffIso gffGeneIsoInfoThis, int boundMaxFalseGapBp) {
